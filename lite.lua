...
AutoparryTab = Window:create_tab("Main", "rbxassetid://10734975692")
...
System = {
    ...
    __config = {
        __curve_names = {'Camera', 'Dot', 'Up', 'Delay', 'Random'},
        __detections = {
...
assert(hook, "exec not supported")
...
function System.auto_spam.spam_service(self)
    ...
    local entityProps = self.Entity_Properties
    local ballProps = self.Ball_Properties
    if not entityProps or not ballProps then return 0 end

    local ping = self.Ping or 0
    local baseRadius = ping + math.min(speed / 6, 95)

    local characterPos = LocalPlayer.Character.PrimaryPart.Position
    local ballPos = ball.Position
    local targetPos = target.PrimaryPart.Position

    local toPlayerVector = characterPos - ballPos
    if toPlayerVector.Magnitude <= 0 then return 0 end

    local toPlayer = toPlayerVector.Unit
    local dot = toPlayer:Dot(velocity.Unit)

    local distToTarget = LocalPlayer:DistanceFromCharacter(targetPos)

    if entityProps.distance > baseRadius then return 0 end
    if ballProps.Distance > baseRadius then return 0 end
    if distToTarget > baseRadius then return 0 end

    local dotBonus = 5 - math.min(speed / 5, 5)
    local dotPenalty = math.clamp(dot, -1, 0) * dotBonus

    local spamRadius = baseRadius - dotPenalty
    return spamRadius * (System.__properties.__auto_spam_distance_multiplier or 1)
end
...
function System.auto_spam.start()
    ...
    System.__properties.__connections.__auto_spam = RunService.PreSimulation:Connect(function()
        ...
        local pingAdjusted = math.clamp(pingMs / 10, 1, 16)
        local ballTarget = ball:GetAttribute("target")
        local ballProperties = System.auto_spam:get_ball_properties()
        local entityProperties = System.auto_spam:get_entity_properties()
        if not ballProperties or not entityProperties then return end
        local spamDist = System.auto_spam.spam_service({
            Ball_Properties = ballProperties,
            Entity_Properties = entityProperties,
            Ping = pingAdjusted
        })
        if type(spamDist) ~= "number" or spamDist <= 0 then return end
        local targetPos = target.PrimaryPart.Position
        local distToTarget = LocalPlayer:DistanceFromCharacter(targetPos)
        local distToBall = LocalPlayer:DistanceFromCharacter(ball.Position)
        if not ballTarget then return end
        if distToTarget > spamDist or distToBall > spamDist then return end
        local isPulsed = LocalPlayer.Character:GetAttribute("Pulsed")
        if isPulsed then return end
        if ballTarget == LocalPlayer.Name and distToTarget > 30 and distToBall > 30 then return end
        if distToBall <= spamDist and System.__properties.__parries > System.__properties.__spam_threshold then
...
autoparry_module:create_dropdown({
    ...
    callback = function(value)
        if not System or not System.__config or not System.__config.__curve_names then return end
        if value == nil then return end
        if type(value) == "number" then
            if value >= 1 and value <= #System.__config.__curve_names then
...
detections_module = AutoparryTab:create_module({
    title = "Detections",
    description = "Skip auto parry during abilities",
...
semi_immortal_module = AutoparryTab:create_module({
    ...
    description = "Best Semi Immortality",
...
semi_immortal_module:create_slider({
    title = "Depth",
    flag = "ZX_SemiImmortalDepth",
    minimum_value = -75,
    maximum_value = 75,
    value = getgenv().SemiImmortalityConfig.Depth,
    round_number = true,
    callback = function(value) getgenv().SemiImmortalityConfig.Depth = value end,
})

semi_immortal_module:create_slider({
    title = "Angle",
    flag = "ZX_SemiImmortalAngle",
    minimum_value = 1,
    maximum_value = 200,
    value = getgenv().SemiImmortalityConfig.Angle,
    round_number = true,
    callback = function(value) getgenv().SemiImmortalityConfig.Angle = value end,
})
